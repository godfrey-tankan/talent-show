...
def employee_record(request, pk):
    if request.user.is_authenticated:
        ...
        return render(
            request, "employee_record.html", {"employee_record": employee_record}
        )
...
def department_record(request, pk):
    if request.user.is_authenticated:
        ...
        return render(
            request, "department_record.html", {"department_record": department_record}
        )
    ...
    if request.user.is_authenticated:
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                record.save()
                return HttpResponse(
                    "Company record uploaded via bulk upload with id "
                    + str(record.pk)
                    + " file: "
                    + str(record.bulk)
                )
...
def search_results(request):
    ...
    results = EmployeeRecord.objects.filter(employee_name__icontains=search_query)
    context = {
        "results": results,
